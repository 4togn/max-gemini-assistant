...
•   • •     •   •  •  •  ••  •  
 •••  ••••• •   • ••• •   • •••
          ...
          │
          ▼
[3] ВЫХОД В ЧАТ: Готовая карточка
...
3. Создайте `.env` на сервере (Бот создаст его автоматически предварительно вставив ключи из `.env.example`):
```bash
cp .env.example .env
nano .env
```
...
```text
Используя инструмент статус будет всё время "В сети". Но используя инструмент неделю, у меня не возникло проблем с аккаунтом, доставкой сообщений. НО, я не хочу чтоб у вас были проблемы с баном, так что не даю 100% гарантии безопасности.
```
---
