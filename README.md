...
[Телефон пользователя]
       │
       ▼ (текстовый запрос в «Избранное»)
[MAX Web (web.max.ru)] ◄─── Постоянная сессия Playwright Chromium
       │
       ▼ (извлечение нового data-index)
[Gemini 3.8 Flash API] ───► Развернутый Markdown + LaTeX ответ
       │
       ▼ (HTML + KaTeX + highlight.js)
[Рендерер карточек WebP] ───► Сжатый 2x Retina RGB WebP (~30 Кб)
       │
       ▼ (загрузка через меню прикрепления)
