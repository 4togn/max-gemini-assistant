...
1. ТЕКСТОВЫЙ РЕЖИМ (Text ➔ WebP Card)

MAX CHAT (Input)                [ Gemini API ]                 MAX CHAT (Output)
   User Prompt (Text)             (gemini-3.8-flash)              Model Reply (WebP Card)
...
2. МУЛЬТИМОДАЛЬНЫЙ РЕЖИМ (Image + Prompt ➔ WebP Card)

MAX CHAT (Input)                [ Gemini API ]                 MAX CHAT (Output)
   Image + Prompt (Text)          (Multimodal Vision)             Analysis & Solution
...
[Телефон пользователя]
       │
       ▼ (текст или фото в «Избранное»)
[MAX Web (web.max.ru)] ◄─── Фоновая сессия Playwright Chromium
       │
       ▼ (извлечение нового data-index и вложений)
[Gemini 3.8 Flash API] ───► Развернутый Markdown + LaTeX ответ
       │
       ▼ (HTML + KaTeX + highlight.js)
[Рендерер карточек WebP] ───► Сжатый 2x Retina RGB WebP (~30-50 Кб)
       │
       ▼ (загрузка карточки через меню прикрепления)
[Отправка карточки в чат MAX]
```
